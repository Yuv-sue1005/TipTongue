...
def parse_args():
    ...
    parser.add_argument('--input_run', required=True, help="pkl input file")
...
answers = {qid:[] for qid in queries}

K = 1000

true = pickle.load(open("top{}.test.reranked.robin.pkl".format(K), 'rb'))
...
def rerank(query, item_list):
    prompt = "I am going to give you a question and a list of items. Give me the top 10 items FROM THE LIST according to the likelihood that the question refers to the item.  " \
             "Format the answer as a numbered list of 10 items. Keep the item names I give you. " \
             "Be direct and return the list ONLY. Stick with items from the list ONLY. Return a list of 10 items ALWAYS. \n\nQUESTION: {}\n\nITEM LIST: {}".format(query, '\n'.join(item_list))
...
for i, qid in enumerate(queries):
    if qid not in true:
        ...
        if qrel[qid] in top1000:
            for item_batch in batch(items, 100):
                answers[qid] = answers[qid] + rerank(query, item_batch)
...
pickle.dump(answers, open("top{}.test.reranked.robin.pkl".format(K), 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
